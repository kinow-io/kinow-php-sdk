...
class GiftsApi
{
    ...
    /**
     * Operation getGiftByToken
     *
     * 
     *
     * @param string $token Gift token to fetch (required)
     * @throws \Kaemo\Client\ApiException on non-2xx response
     * @return \Kaemo\Client\Model\Gift
     */
    public function getGiftByToken($token)
    {
        list($response) = $this->getGiftByTokenWithHttpInfo($token);
        return $response;
    }

    /**
     * Operation getGiftByTokenWithHttpInfo
     *
     * 
     *
     * @param string $token Gift token to fetch (required)
     ...
     * @return array of \Kaemo\Client\Model\Gift, HTTP status code, HTTP response headers (array of strings)
     */
    public function getGiftByTokenWithHttpInfo($token)
    {
        // verify the required parameter 'token' is set
        if ($token === null) {
            throw new \InvalidArgumentException('Missing the required parameter $token when calling getGiftByToken');
        }
        // parse inputs
        $resourcePath = "/gifts/{token}";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
